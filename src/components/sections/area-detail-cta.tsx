import Link from "next/link";
import { ArrowRight } from "lucide-react";
import { Container } from "@/components/layout";
import { getWhatsAppLinkWithContext } from "@/lib/whatsapp";

interface AreaDetailCtaProps {
  areaTitle: string;
}

export function AreaDetailCta({ areaTitle }: AreaDetailCtaProps) {
  return (
    <section className="py-20 md:py-28 bg-ice">
      <Container size="small">
        <div className="bg-navy relative overflow-hidden">
          {/* Decorative line */}
          <div className="absolute top-0 left-0 right-0 h-px bg-bronze/40" />
          
          <div className="p-10 md:p-14 text-center text-white relative">
            {/* Editorial label */}
            <div className="flex items-center justify-center gap-3 mb-6">
              <div className="w-8 h-px bg-bronze" />
              <p className="text-bronze font-medium tracking-widest uppercase text-xs">
                Consultoria Especializada
              </p>
              <div className="w-8 h-px bg-bronze" />
            </div>
            
            <h2 className="font-serif text-2xl md:text-3xl lg:text-4xl font-medium mb-4">
              Precisa de orientação em{" "}
              <span className="italic text-bronze">{areaTitle}</span>?
            </h2>
            
            <p className="text-white/60 mb-10 max-w-lg mx-auto">
              Entre em contato para uma análise inicial da sua situação.
              Atendimento técnico, ético e comprometido com a defesa dos seus direitos.
            </p>

            {/* CTAs */}
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
              <a
                href={getWhatsAppLinkWithContext(areaTitle)}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-bronze text-white font-medium hover:bg-bronze-hover transition-colors"
              >
                Falar no WhatsApp
              </a>
              <Link
                href="/contato"
                className="inline-flex items-center justify-center gap-2 px-8 py-4 border border-white/30 text-white font-medium hover:bg-white/5 transition-colors"
              >
                Formulário de Contato
                <ArrowRight className="w-4 h-4" />
              </Link>
            </div>

            {/* Microcopy */}
            <p className="text-xs text-white/40">
              Atendimento mediante agendamento. Sigilo garantido.
            </p>
          </div>
        </div>
      </Container>
    </section>
  );
}
